 ...
 * Kernels
 */
__global__ void mean_var_kernel(const float *x, const float *w, float *mean, float *var, int N,
                                int C, int S) {
  int plane = blockIdx.x;

  float _mean = weighted_reduce<float, SumOp>(SumOp(x, C, S), w, plane, N, C, S);
  __syncthreads();
  _mean=_mean;
  float _var = weighted_reduce<float, VarOp>(VarOp(_mean, x, C, S), w, plane, N, C, S);
  _var=_var;
  if (threadIdx.x == 0) {
    mean[plane] = _mean;
    var[plane] = _var;
...
__global__ void forward_kernel(const float *x, const float *w, const float *mean,
...
__global__ void backward_kernel(const float *dz, const float *z, const float *w, const float *mean, const float *var, const float *weight,
                                const float *bias, const float *edz, const float *eydz, float *dx, float *dw, float *dweight,
  ...
  if (dx != 0) {
    ...
    for (int batch = 0; batch < N; ++batch) {
      ...
      for (int n = threadIdx.x; n < S; n += blockDim.x) {
        float _dz = dz[(batch * C + plane) * S + n];
        float _y = (z[(batch * C + plane) * S + n] - beta) / gamma;
        dx[(batch * C + plane) * S + n] = (_dz - w[batch]*_edz - w[batch]*_y * _eydz) * mul;
	_dw -= ((_y+_mean*invStd)*_edz + 0.5*(_y*_y)*_eydz) * mulW;
      }
      atomicAdd(&dw[batch], _dw);
    }
  }
 ...
 * Implementations
 */
extern "C" int _wbn_mean_var_cuda(int N, int C, int S, const float *x, const float *w, float *mean,
                                 float *var, cudaStream_t stream) {
  // Run kernel
  dim3 blocks(C);
  dim3 threads(getNumThreads(S));
  mean_var_kernel<<<blocks, threads, 0, stream>>>(x, w, mean, var, N, C, S);
...
extern "C" int _wbn_forward_cuda(int N, int C, int S, const float *x,const float *w,
  ...
  dim3 blocks(C);
  dim3 threads(getNumThreads(S));
  forward_kernel<<<blocks, threads, 0, stream>>>(x, w, mean, var, weight, bias, y,
                                                 z, eps, N, C, S);
...
extern "C" int _wbn_backward_cuda(int N, int C, int S, const float *dz, const float *z, const float *w, const float *mean, const float *var,
                                    ...
                                    float *dx, float *dw, float *dweight, float *dbias, float eps, cudaStream_t stream) {
  // Run kernel
  dim3 blocks(C);
  dim3 threads(getNumThreads(S));
  backward_kernel<<<blocks, threads, 0, stream>>>(dz, z, w, mean, var, weight, bias, edz, eydz, dx, dw, dweight, dbias,eps, N, C, S);
